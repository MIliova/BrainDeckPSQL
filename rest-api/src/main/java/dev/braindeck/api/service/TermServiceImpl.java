...
public class TermServiceImpl implements TermService {
    ...
    @Override
    public NewDTermDto autoCreate(int userId, int setId, DTermPayload payload) {
    ...
    @Override
    @Transactional
    public void autoUpdate(int termId, int setId, int currentUserId, DTermPayload payload) {
        TermEntity term = termRepository.findById(termId)
                .orElseThrow(() -> new NoSuchElementException("errors.term.not.found"));

        checkOwnership(term, setId, currentUserId);
    ...
    @Override
    public List<TermDto> autoCreate(int userId, int setId, List<DTermPayload> payloads) {
        ...
        for (DTermPayload p : payloads) {
            list.add(new TermEntity(p.term(), p.description(), set));
        }
        termRepository.saveAll(list);
        return Mapper.termsToDto(list);
    }
